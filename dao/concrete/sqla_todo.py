...
from dao.interfaces import TodoDAOInterface, TodoAdminDAOInterface
...
class SQLTodoDAO(TodoDAOInterface):
    ...
    async def update(self, todo_id: int, user_id: int, data: TodoUpdate) -> Optional:
        print("inf func")
        result = await self.session.execute(
        ...
        return await self.get_by_id(todo_id=todo_id, user_id=user_id)
...
class SQLTodoAdminDAO(TodoAdminDAOInterface):
    def __init__(self, session: AsyncSession):
        self.session = session
